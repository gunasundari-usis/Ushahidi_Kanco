...
class Reports_Controller extends Admin_Controller
{
    ...
    */
	...
	function download()
	{
		...
		$errors = $form;
		$form_error = FALSE;
                $disp_custom_fields = $this->_get_custom_form_fields(false,1,false);
                $between = 'between[1,'.sizeof($disp_custom_fields).']';
	   // print_r( sizeof($disp_custom_fields) ); exit;
		// check, has the form been submitted, if so, setup validation
	    if ($_POST)
	    {
            // Instantiate Validation, use $post, so we don't overwrite $_POST fields with our own things
	        ...
	        $post->add_rules('data_point.*','required','numeric','between[1,4]');
                $post->add_rules('data_include.*','numeric','between[1,19]');
                $post->add_rules('custom_form_include.*','numeric',$between);
		...
		$post->add_rules('to_date','date_mmddyyyy');
	        ...
	        if ($post->validate())
	        {
				...
				// Column Titles
				$report_csv = "#,PROFILE TITLE,PROFILE DATE";
				foreach($post->data_include as $item)
				{
					...
					if ($item == 3) {
						$report_csv .= ",CATEGORY";
                                ...
                                foreach($disp_custom_fields as $disp_custom_field ) {
                                    $report_csv .= ','.strtoupper($disp_custom_field['field_name']).'';
                                }
				...
				foreach ($incidents as $incident)
				{
					...
					$form_items = $this->_get_custom_form_fields($incident->id,1,true);
					foreach($post->data_include as $item)
					{
						if ($item == 1) {
						...
						if ($item == 3) {
							...
							$report_csv .= '"';
                                        ...
                                        foreach($form_items as $form_item ) {
                                            $report_csv .= ',"'.htmlspecialchars($form_item).'"';
                                        }
    ...
    */
	function deletePhoto ( $id )
	{
        ...
        function delete_docs($id)
        {
            ...
            if( $id )
            {
                $docs = ORM::factory('docs',$id);
                $doc_item = $docs->docs_link;
            }
            //Delete doc files from directory
            if( !empty( $doc_item ) ) 
                unlink(Kohana::config('upload.directory',TRUE) . $doc_item );
